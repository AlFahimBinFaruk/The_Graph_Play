...
describe("Task3", function () {
  ...
  describe("Create", function () {
    it("Allows only the owner to update the creation hash", async () => {
      ...
      assert.equal(creationHash, "0x1234567890abcdef1234567890abcdef12345678");
    });
    ...
    it("Reverts if non-owner tries to update creation hash", async () => {
      await expect(
        task3
          ...
          .create(0, "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd")
      ).to.be.revertedWithCustomError(task3, "OwnableUnauthorizedAccount");
    });
  ...
  describe("Update", function () {
    it("Allows only the owner to update the update hash", async () => {
      ...
      assert.equal(updateHash, "0x1234567890abcdef1234567890abcdef12345678");
    });
    ...
    it("Reverts if non-owner tries to update update hash", async () => {
      await expect(
        task3
          .connect(otherAccount)
          .update(0, "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd")
      ).to.be.revertedWithCustomError(task3, "OwnableUnauthorizedAccount");
    });
